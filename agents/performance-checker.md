...
TypeORM에서 관계 엔티티 접근 시 N+1 패턴을 감지한다.
...
목록을 반환하는 API에 Pagination이 없으면 감지한다.
...
WHERE/ORDER BY에 사용되는 컬럼에 인덱스가 없으면 경고한다.
...
문자열 DTO 필드에 `@MaxLength()`가 없으면 경고한다 (OWASP API4:2023).
...
빈번 조회 데이터(설정, 목록)에 캐싱이 없으면 제안한다.
...
API 엔드포인트에 대한 Integration Test에 응답 시간 검증이 없으면 경고한다.
...
## trine-performance.md 룰 참조

이 에이전트는 `~/.claude/trine/rules/trine-performance.md`에 정의된 규칙을 검증한다.
